...
const diffRun = require("./lib/diff-run");
...
nodecg.listenFor('updateSchedule', (_data, cb) => {
    nodecg.log.trace('Manual Update for schedule');
...
nodecg.listenFor('modifyRun', (data, cb) => {
    ...
    if (run) {
        ...
        if (original) {
            ...
            assign(run, data);
            run.originalValues = diffRun.calcOriginalValues(run, original);
        }
        else {
...
/**
 * Gets the latest schedule info from the tracker.
            ...
            if (currentRunAsInSchedule) {
                [currentRunRep, nextRunRep].forEach(activeRunReplicant => {
                    if (activeRunReplicant.value && activeRunReplicant.value.pk) {
                        ...
                        if (runFromSchedule) {
                            activeRunReplicant.value = diffRun.mergeChangesFromTracker(activeRunReplicant.value, runFromSchedule);
                        }
                    }
                });
            }
...
/**
 * Formats a raw run object from the Tracker API into a slimmed-down and hydrated version for our use.
 * @param rawRun - A raw run object from the Tracker API.
